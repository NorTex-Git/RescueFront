...
export async function fetchHardwareByEmpresa(empresaId: string): Promise<Hardware[]> {
...
const hardwareTypesSchema = z.object({
  success: z.boolean(),
  data: z
    .array(z.object({ nombre: z.string().default(''), name: z.string().optional() }))
    .default([]),
})

export async function fetchHardwareTypeOptions(): Promise<FieldOption[]> {
  const raw = await apiFetch<unknown>('/api/hardware-types')
  return hardwareTypesSchema
    .parse(raw)
    .data.map((item) => item.nombre || item.name || '')
    .filter(Boolean)
