...
export default async function CompanyTypesPage() {
  const tiposLoad = await preload('tipos de empresa', () =>
    apiFetch<unknown>('/api/tipos_empresa/dashboard/all').then(
      (raw) => companyTypesListSchema.parse(raw).data,
    ),
  )
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      <CompanyTypesView initialLoad={tiposLoad} />
    </div>
  )
}
